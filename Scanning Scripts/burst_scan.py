...
# --- GLOBAL WRITER QUEUE ---
# This queue holds images in RAM until the background thread saves them.
write_queue = queue.Queue()

def writer_worker():
    """Background thread that saves images from RAM to Disk."""
    while True:
        ...
        image, filename = item
        try:
            # This is the slow blocking operation, now offloaded to this thread
            cv2.imwrite(filename, image)
        except Exception as e:
            print(f"  [ERROR] Background write failed for {filename}: {e}")
...
def init_camera_manual():
    ...
    # RGB888 format matches the corrected color behavior
...
def save_scan_parameters(output_dir, start_pos, cam_settings):
    ...
    try:
        with open(filepath, "w") as f:
            f.write("=== VERBOSE SCAN PARAMETERS (BURST MODE) ===\n")
...
def perform_burst_stack(cam, output_folder, original_z):
    """Captures a sequence of images while moving Z."""
    
    # Create subfolder for this specific point
    ...
    # Info about queue backlog
    backlog = write_queue.qsize()
    print(f"    [BURST] Capturing {total_frames} frames. (Write Backlog: {backlog} images)")

    # --- Start Z Move ---
    send_gcode("G91") 
    # No M400: Move immediately
    ...
    for i in range(total_frames):
        ...
        # 1. Capture Image Array (Fast)
        image = cam.capture_array("main")
        
        # 2. Queue Image for Background Writing (Instant)
        # We do NOT call cv2.imwrite here anymore.
        filename = os.path.join(output_folder, f"frame_{i:05d}.jpg")
        write_queue.put((image, filename))
        
        # 3. Throttle to maintain FPS
    ...
    # --- End Z Move ---
    send_gcode("M400") # Now we wait for physical move to finish
    ...
    print(f"    [BURST] Capture Loop finished in {actual_duration:.2f}s. Writing continues in background.")
...
def main():
    ...
    writer_thread.start()
    print("  [SYSTEM] Background image writer started.")
    ...
    try:
        ...
        for h_step in range(STEPS_H):
            ...
            for w_step in w_range:
                ...
                perform_burst_stack(camera, stack_folder, start_z)
        ...
        # Wait for the background queue to finish writing everything
        remaining = write_queue.qsize()
        if remaining > 0:
            print(f"\n  [SYSTEM] Finishing writes for {remaining} images...")
            print("  (Do not power off)")
        
        write_queue.join()
        print("  [SYSTEM] All images saved.")
